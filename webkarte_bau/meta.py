...
GRUPPEN = [
    {"id": "importance", "label": "Netzbedeutung", "kurz": "Link Importance",
     "rolle": "importance",
     "text": "Link Importance nach Jenelius et al.: durchschnittlicher Reisezeitanstieg in Sekunden je relevantem "
             "Quell-Ziel-Paar, wenn die Kante gesperrt wird und die kürzesten Reisezeiten neu berechnet werden. Quell- und "
             "Zielpunkte sind die 129 Demand Nodes (Gemeindezentren). Nur 4.212 der 72.238 Kanten sind aktiv "
             "(Link Importance > 0)."},
    {"id": "pluvial", "label": "Starkregen", "kurz": "pluvial", "rolle": "pluvial",
     "text": "BKG-Hinweiskarte Starkregengefahren, extremes Ereignis (100 mm/h, 1-m-Raster), Abtastung im Meterabstand "
             "entlang jeder Kante. Gefährdungsstufen H1 bis H6 nach dem H-Schema aus Überflutungstiefe × Fließgeschwindigkeit "
             "(ortsgekoppelt). Verschneidungsindex vi_pct = Produkt der Perzentilränge von Gefahrengröße und Link Importance "
             "innerhalb der betroffenen und aktiven Kanten."},
    {"id": "fluvial", "label": "Flusshochwasser", "kurz": "fluvial", "rolle": "fluvial",
     "text": "Hochwassergefahrenkarten HQextrem des NLWKN für die Risikogewässer Hase und Hunte. Die fluviale "
             "Gefährdungsstufe folgt aus der klassierten Tiefe, ohne Fließgeschwindigkeit. Brückenfreier Verschneidungsindex "
             "vi_pct_fluvial über 138 Kanten. Kanten, die nur in Überschwemmungsgebieten ohne Tiefenangabe liegen, sind "
             "Prüffälle und gehen nicht in den Index ein."},
    {"id": "heat", "label": "Hitze", "kurz": "heat", "rolle": "heat",
     "text": "Hitzewellenlage: 90. Perzentil der Landoberflächentemperatur aus Landsat 8 und 9 (Komposit), Zonenstatistik im "
             "50-m-Puffer je Kante. Thermischer Verschneidungsindex vi_pct_heat = Produkt der Perzentilränge von Hitzewellenlage "
             "und Link Importance über die aktiven Kanten. Belag, Beton und Beschattung beschreiben die materielle Anfälligkeit; "
             "Baumbedeckung und Versiegelung im Straßenumfeld erklären die Temperaturunterschiede."},
    {"id": "compound", "label": "Compound", "kurz": "compound", "rolle": "compound",
     "text": "Zusammentreffen zweier Gefahren auf derselben Kante. Der Compound-Index der Wassergefahren verknüpft die "
             "Perzentilränge des pluvialen Verschneidungsindex und der fluvialen Gefährdungsstufe (vi_pct_compound). "
             "Ergänzend dieselbe Verknüpfung für Starkregen und Hitze (vi_pct_compound_heat)."},
    {"id": "profil", "label": "Belastungsprofil", "kurz": "Mehrfachbelastung", "rolle": "compound",
     "text": "Gefahrenübergreifendes Belastungsprofil: Zahl der Gefahren, in denen eine Kante zum obersten Dezil ihres "
             "Verschneidungsindex gehört. mhn_bf = Fassung der "
    ...
    {"id": "kontext", "label": "Kanteneigenschaften", "kurz": "Kontext", "rolle": "importance",
     "text": "Straßentyp, Baulastebene, Gemeinde und Länge aus OpenStreetMap bzw. Zuordnung."},
]
...
VARIABLEN = [
    ...
    ("importance_s", "importance", "kanten.json", "Link Importance (Sekunden)", "s", "kontinuierlich", "importance", True, None,
     "Durchschnittlicher Reisezeitanstieg je relevantem Quell-Ziel-Paar bei Sperrung der Kante."),
    ("imp_pct", "importance", "attr_importance.json", "Link Importance (Perzentilrang 0–1)", "", "kontinuierlich", "importance", False, None,
     "Perzentilrang 0–1; geht in die Verschneidungsindizes ein."),
    ("klasse", "importance", "attr_importance.json", "Klasse der Link Importance", "", "kategorial", "importance", False,
    ...
    ("hazard_klasse", "pluvial", "kanten.json", "Starkregen-Gefährdungsstufe", "", "kategorial", "pluvial", False,
     ["H1", "H2", "H3", "H4", "H5", "H6", "querbauwerk"], "H1 bis H6 nach dem H-Schema (Überflutungstiefe × Fließgeschwindigkeit); Querbauwerke (Brücken, Tunnel) gesondert; ohne Stufe = außerhalb der Abtastung."),
    ("vi_pct", "pluvial", "attr_pluvial.json", "Starkregen-Verschneidungsindex", "", "kontinuierlich", "pluvial", True, None,
     "Produkt der Perzentilränge von Link Importance (imp_pct_pluvial) und Gefahrengröße (dv_pct) innerhalb der pluvial betroffenen und aktiven Kanten; oberstes Dezil = Spitzengruppe."),
    ("vi_roh", "pluvial", "attr_pluvial.json", "Starkregen-Verschneidungsindex, unnormierte Fassung", "s·m²/s", "kontinuierlich", "pluvial", True, None,
     "importance_s × dv_max; wird der perzentilnormierten Fassung gegenübergestellt."),
    ...
    ("tiefe_max_m", "pluvial", "attr_pluvial.json", "max. Überflutungstiefe", "m", "kontinuierlich", "pluvial", True, None,
     "Größte Überflutungstiefe entlang der Kante (Abtastung im Meterabstand)."),
    ("flut_anteil", "pluvial", "attr_pluvial.json", "überfluteter Anteil", "", "kontinuierlich", "pluvial", True, None,
     "Anteil nasser Stützpunkte an allen bewerteten Stützpunkten."),
    ("h_klasse_fl", "fluvial", "attr_fluvial.json", "Fluviale Gefährdungsstufe (HQextrem)", "", "kategorial", "fluvial", False,
     ["H2", "H3", "H4", "H5", "H6"], "Gefährdungsstufe aus der klassierten Tiefe der HQextrem-Fläche (reine Tiefenschwellen, ohne Fließgeschwindigkeit)."),
    ("vi_pct_fluvial", "fluvial", "attr_fluvial.json", "Fluvialer Verschneidungsindex (brückenfrei)", "", "kontinuierlich", "fluvial", True, None,
     "Produkt der Perzentilränge von Link Importance und fluvialer Gefährdungsstufe, ohne Querbauwerke, 138 Kanten (Endergebnis der Arbeit für diese Gefahr)."),
    ("vi_pct_fluvial_voll", "fluvial", "attr_fluvial.json", "Fluvialer Verschneidungsindex (mit Querbauwerken)", "", "kontinuierlich", "fluvial", True, None,
     "Zwischenstand einschließlich Querbauwerken (n = 197)."),
    ("vi_roh_fluvial_nb", "fluvial", "attr_fluvial.json", "Fluvialer Verschneidungsindex, unnormierte Fassung", "", "kontinuierlich", "fluvial", True, None,
     "importance_s × Gefährdungsstufe (brückenfrei); wird der perzentilnormierten Fassung gegenübergestellt."),
    ("tiefe_repr_fl", "fluvial", "attr_fluvial.json", "Tiefe der HQextrem-Fläche (Klassenobergrenze)", "m", "kontinuierlich", "fluvial", True, None,
     "Obergrenze der Tiefenklasse der HQextrem-Fläche, in der die Kante liegt."),
    ...
    ("fluvial_status", "fluvial", "attr_fluvial.json", "Fluvialer Status", "", "kategorial", "fluvial", False,
     ["Tiefe HQextrem", "nur Ausdehnung"], "Tiefe aus der HQextrem-Gefahrenkarte vorhanden oder nur Ausdehnung ohne Tiefe (Überschwemmungsgebiet, Prüffall)."),
    ...
    ("lst_p90_mean", "heat", "attr_heat.json", "Hitzewellenlage (LST, 90. Perzentil)", "°C", "kontinuierlich", "heat", False, None,
     "Mittel der Hitzewellenlage (90. Perzentil des Landsat-Komposits) im 50-m-Puffer, alle Kanten."),
    ("heat_pct", "heat", "attr_heat.json", "Thermische Exposition (Perzentilrang)", "", "kontinuierlich", "heat", True, None,
     "Perzentilrang der Hitzewellenlage über die aktiven Kanten."),
    ("vi_pct_heat", "heat", "attr_heat.json", "Thermischer Verschneidungsindex", "", "kontinuierlich", "heat", True, None,
     "Produkt der Perzentilränge von Hitzewellenlage (heat_pct) und Link Importance (imp_pct) über die aktiven Kanten."),
    ("vi_pct_heat_v2", "heat", "attr_heat.json", "Thermischer Verschneidungsindex, Bewertungsvariante Belag", "", "kontinuierlich", "heat", True, None,
     "Bewertungsvariante, die den Belag zusätzlich als Anfälligkeitsfaktor gewichtet."),
    ("vi_pct_heat_v3", "heat", "attr_heat.json", "Thermischer Verschneidungsindex, Bewertungsvariante Beschattung", "", "kontinuierlich", "heat", True, None,
     "Bewertungsvariante, die die Beschattung zusätzlich als Anfälligkeitsfaktor gewichtet."),
    ("vi_pct_heat_v4", "heat", "attr_heat.json", "Thermischer Verschneidungsindex, Bewertungsvariante Belag und Beschattung", "", "kontinuierlich", "heat", True, None,
     "Bewertungsvariante mit Belag und Beschattung als Anfälligkeitsfaktoren."),
    ("robust_n", "heat", "attr_heat.json", "Variantenrobustheit (Bewertungsvarianten in den Top 25)", "", "kategorial", "heat", False,
     ["0", "1", "2", "3", "4"], "In wie vielen der vier Bewertungsvarianten die Kante unter den Top 25 liegt; die 23 Kanten in allen vier bilden den thermischen Robustheitskern (varianteninvariante Kernmenge)."),
    ("tcd_mean", "heat", "attr_heat.json", "Baumbedeckung", "%", "kontinuierlich", "coverage", False, None,
     "Baumbedeckung im Straßenumfeld (Copernicus HRL Tree Cover Density, Kantenpuffer)."),
    ("imd_mean", "heat", "attr_heat.json", "Versiegelungsgrad", "%", "kontinuierlich", "importance", False, None,
     "Versiegelung im Straßenumfeld (Copernicus HRL Imperviousness Density, Kantenpuffer)."),
    ("surface_class", "heat", "attr_heat.json", "Fahrbahnbelag", "", "kategorial", "heat", False,
     ["asphalt", "concrete", "pflaster", "befestigt_unspez", "ungebunden", "sonstig"], "Fahrbahnbelag aus OpenStreetMap (surface); Kanten ohne Angabe sind als asphaltiert approximiert."),
    ("beschattung", "heat", "attr_heat.json", "Beschattungsklasse", "", "kategorial", "coverage", False,
     ["unbeschattet", "gering", "mittel", "hoch"], "Klassen aus der Baumbedeckung im Straßenumfeld."),
    ("vi_pct_compound", "compound", "attr_compound.json", "Compound-Index der Wassergefahren (Starkregen × Flusshochwasser)", "", "kontinuierlich", "compound", True, None,
     "Verknüpft den Perzentilrang des pluvialen Verschneidungsindex mit dem der fluvialen Gefährdungsstufe (n = 119)."),
    ("vi_roh_compound", "compound", "attr_compound.json", "Compound-Index der Wassergefahren, unnormierte Fassung", "", "kontinuierlich", "compound", True, None, ""),
    ("vi_pct_compound_heat", "compound", "attr_compound.json", "Compound-Index Starkregen × Hitze", "", "kontinuierlich", "compound", True, None,
     "Produkt der Perzentilränge von Starkregen und Hitze (n = 3.454)."),
    ...
    ("baulast", "kontext", "kanten.json", "Baulastebene", "", "kategorial", "importance", False,
     ["autobahn", "bundesstrasse", "landesstrasse", "kreisstrasse", "gemeindestrasse"], "Zuordnung nach Straßennummer und Straßentyp."),
...
SPALTEN = {
    ...
    "highway": ("Straßentyp", "", None, "kontext"), "baulast": ("Baulastebene", "", None, "kontext"),
    ...
    "importance_s": ("Link Importance", "s", 4, "importance"), "imp_pct": ("Perzentilrang Link Importance", "", 4, "importance"),
    "imp_pct100": ("Perzentilrang Link Importance (0–100)", "%", 1, "importance"), "klasse": ("Klasse der Link Importance", "", None, "importance"),
    "hazard_klasse": ("Gefährdungsstufe", "", None, "pluvial"), "tiefe_max_m": ("max. Überflutungstiefe", "m", 2, "pluvial"),
    ...
    "imp_pct_pluvial": ("Perzentilrang Link Importance (pluviale Bezugsmenge)", "", 4, "pluvial"),
    "dv_pct": ("Perzentilrang Gefahrengröße", "", 4, "pluvial"), "vi_pct": ("Starkregen-Verschneidungsindex", "", 4, "pluvial"),
    "vi_roh": ("Starkregen-Index, unnormiert", "", 3, "pluvial"), "rang_pct": ("Rang (vi_pct)", "", 0, "pluvial"),
    "fluvial_status": ("Fluvialer Status", "", None, "fluvial"), "usg_betroffen": ("im Überschwemmungsgebiet", "", None, "fluvial"),
    "pruefbedarf": ("Prüffall", "", None, "fluvial"), "querungspunkt": ("Gewässerquerung", "", None, "fluvial"),
    "gewaesser_usg": ("Gewässer (Überschwemmungsgebiet)", "", None, "fluvial"), "betroffen_fl": ("in HQextrem-Fläche", "", None, "fluvial"),
    "h_klasse_fl": ("Gefährdungsstufe (HQextrem)", "", None, "fluvial"), "h_num_fl": ("Gefährdungsstufe (numerisch)", "", 0, "fluvial"),
    "tiefe_repr_fl": ("Tiefe (Klassenobergrenze)", "m", 2, "fluvial"), "ufl_m": ("überflutete Länge", "m", 0, "fluvial"),
    "imp_pct_nb": ("Perzentilrang Link Importance (fluvial)", "", 4, "fluvial"), "haz_pct_nb": ("Perzentilrang Gefährdungsstufe (fluvial)", "", 4, "fluvial"),
    "vi_pct_fluvial": ("Fluvialer Verschneidungsindex (brückenfrei)", "", 4, "fluvial"), "vi_roh_fluvial_nb": ("Fluvialer Index, unnormiert", "", 3, "fluvial"),
    "vi_band": ("Perzentilband", "", 0, "fluvial"), "vi_pct_fluvial_voll": ("Fluvialer Index (mit Querbauwerken)", "", 4, "fluvial"),
    "lst_p90_mean": ("Hitzewellenlage (LST P90)", "°C", 2, "heat"), "tcd_mean": ("Baumbedeckung", "%", 1, "heat"),
    "imd_mean": ("Versiegelung", "%", 1, "heat"), "surface_class": ("Fahrbahnbelag", "", None, "heat"),
    "beschattung": ("Beschattungsklasse", "", None, "heat"), "concrete_flag": ("Betonbelag", "", None, "heat"),
    "heat_pct": ("Thermische Exposition", "", 4, "heat"), "vi_pct_heat": ("Thermischer Verschneidungsindex", "", 4, "heat"),
    "vi_pct_heat_v2": ("Bewertungsvariante Belag", "", 4, "heat"), "vi_pct_heat_v3": ("Bewertungsvariante Beschattung", "", 4, "heat"),
    "vi_pct_heat_v4": ("Bewertungsvariante Belag und Beschattung", "", 4, "heat"), "robust_n": ("Variantenrobustheit (0–4)", "", 0, "heat"),
    "stark_pct": ("Starkregen-Perzentil", "", 4, "compound"), "vi_pct_compound": ("Compound-Index Wassergefahren", "", 4, "compound"),
    "vi_roh_compound": ("Compound-Index Wassergefahren, unnormiert", "", 3, "compound"), "stark_pct_h": ("Starkregen-Perzentil (Hitze-Compound)", "", 4, "compound"),
    "hitze_pct": ("Hitze-Perzentil", "", 4, "compound"), "vi_pct_compound_heat": ("Compound-Index Starkregen × Hitze", "", 4, "compound"),
    "vi_roh_compound_heat": ("Compound-Index Starkregen × Hitze, unnormiert", "", 3, "compound"),
...
PRESETS = [
    ...
    {"id": "pluvial_hstufen", "titel": "Starkregengefahr nach Gefährdungsstufe (H-Schema)", "variable": "hazard_klasse",
    ...
    {"id": "pluvial_index", "titel": "Perzentilnormierter Starkregen-Verschneidungsindex", "variable": "vi_pct",
    ...
    {"id": "compound", "titel": "Compound-Index der Wassergefahren", "variable": "vi_pct_compound",
...
def variablen_katalog(basis: pd.DataFrame, tabellen: dict[str, pd.DataFrame]) -> list[dict]:
    ...
    for (vid, gruppe, datei, label, einheit, typ, rolle, gt0, werte, text) in VARIABLEN:
        ...
        if typ == "kategorial":
            ...
            vc = vc.astype(str)
            if vid in ("robust_n", "vi_band", "mhn_bf", "multi_hazard_n", "h_num_fl"):
