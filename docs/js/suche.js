...
WK.suche = (() => {
  ...
  function indexBauen() {
    ...
    for (const fe of WK.daten.features) {
      ...
      if (p.ref) { for (const r of String(p.ref).split(';')) { const k = normRef(r); let e = refs.get(k); if (!e) { e = { ref: r.trim(), ids: [] }; refs.set(k, e); } e.ids.push(fe.id); } }
    }
  ...
  function suchen(q) {
    ...
    for (const r of S.index.refs) if (r.k.startsWith(rq) || (rq.length >= 2 && r.k.includes(rq))) aus.push({ art: 'Straßenzug', label: `${r.ref} (${r.ids.length} Kanten)`, ids: r.ids, ref: r.ref });
